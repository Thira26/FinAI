...
public class SmsReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        try {
            Log.d("FinAI", "SMS received: " + intent.getAction());
            if (Telephony.Sms.Intents.SMS_RECEIVED_ACTION.equals(intent.getAction())) {
                Bundle bundle = intent.getExtras();
                if (bundle != null) {
                    for (SmsMessage sms : Telephony.Sms.Intents.getMessagesFromIntent(intent)) {
                        String body = sms.getMessageBody();
                        Log.d("FinAI", "Processing SMS: " + (body != null ? body.substring(0, Math.min(50, body.length())) : "null"));
            ...
            Log.e("FinAI", "Error processing SMS", e);
        }
    }
}