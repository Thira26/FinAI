...
public class AnalyticsFragment extends Fragment {
    ...
    private void refreshAnalytics() {
        ...
        // Insights from last 30 days with better formatting
        String insights = formatInsights(new LocalTransactionsRepository(requireContext()).getAll());
    ...
    private String formatInsights(List<TransactionModel> list) {
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd", Locale.US);
            long today = sdf.parse(sdf.format(new Date())).getTime();
            long dayMs = 24L*60*60*1000;
            ...
            float total = week1 + week2 + week3 + week4;
            if (total == 0) {
                return "No spending data available for the last 30 days.\n\nStart adding transactions to see your spending insights here!";
            }
            
            // Find top category
            String topCat = "-"; float topVal = 0f; float categoryTotal = 0f;
            for (Map.Entry<String, Float> e : cat.entrySet()) { 
                categoryTotal += e.getValue(); 
            ...
            int topPercentage = categoryTotal > 0 ? Math.round((topVal / categoryTotal) * 100) : 0;
            
            // Calculate weekly average
            float weeklyAvg = total / 4;
            
            // Determine trend
            String trend = "";
            if (week4 > week3 && week3 > week2) {
                trend = "Your spending has been increasing over the past 3 weeks.";
            } else if (week4 < week3 && week3 < week2) {
                trend = "Great! Your spending has been decreasing over the past 3 weeks.";
            } else if (week4 > weeklyAvg) {
                trend = "This week's spending is above your 4-week average.";
            } else {
                trend = "This week's spending is below your 4-week average. Keep it up!";
            }
            
            // Build formatted summary
            StringBuilder summary = new StringBuilder();
            summary.append("📊 Spending Summary (Last 30 Days)\n\n");
            
            if (!topCat.equals("-")) {
                String categoryName = topCat.substring(0, 1).toUpperCase() + topCat.substring(1);
                summary.append("💳 Top Category: ").append(categoryName)
                       .append("\n   Spent ₹").append((long)topVal)
                       .append(" (").append(topPercentage).append("% of total)\n\n");
            }
            
            summary.append("📅 Weekly Breakdown:\n");
            summary.append("   Week 1: ₹").append((long)week1).append("\n");
            summary.append("   Week 2: ₹").append((long)week2).append("\n");
            summary.append("   Week 3: ₹").append((long)week3).append("\n");
            summary.append("   Week 4: ₹").append((long)week4).append("\n\n");
            
            summary.append("📈 Total (30 days): ₹").append((long)total).append("\n");
            summary.append("📊 Weekly Average: ₹").append((long)weeklyAvg).append("\n\n");
            
            summary.append("💡 ").append(trend);
